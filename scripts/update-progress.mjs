...
const partStats = {};
for (let partNumber = 0; partNumber <= 7; partNumber++) {
	partStats[partNumber] = { completed: 0, total: 0 };
}
...
for (let partNumber = 0; partNumber <= 7; partNumber++) {
	totalCompleted += partStats[partNumber].completed;
	totalExercises += partStats[partNumber].total;
}
...
for (let partNumber = 0; partNumber <= 7; partNumber++) {
	const { completed, total } = partStats[partNumber];
	const pct = total > 0 ? Math.round((completed / total) * 100) : 0;
	tableRows.push(`| ${partNumber} | ${partTopics[partNumber]} | ${completed} | ${total} | ${pct}% |`);
}
...
for (let partNumber = 0; partNumber <= 7; partNumber++) {
	const { completed, total } = partStats[partNumber];
	const pct = total > 0 ? Math.round((completed / total) * 100) : 0;
	const topic = partTopics[partNumber].length > 45
		? partTopics[partNumber].slice(0, 42) + '...'
		: partTopics[partNumber];
	console.log(
		`Part ${partNumber}  ${topic.padEnd(47)} ${String(completed).padStart(4)} ${String(total).padStart(5)} ${String(pct + '%').padStart(4)}`
	);
}
